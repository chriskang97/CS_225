...
class InorderTraversal : public TreeTraversal<T> {
  public:
	    ...
	    : root(root)
    {
      ...
      // Set Starting Point at leftmost node
      root = stack.top() ;
    ...
    void add(typename BinaryTree<T>::Node *& treeNode) {
      ...
      // If there are any right nodes, traverse through that one and then traverse all the left nodes again.
