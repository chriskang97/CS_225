...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     Point<Dim> quick_select( vector<Point<Dim>> & newPoints, int i, int j, int left_bound, int right_bound, int curr_dim, int desire_index );
     void Build_KD(vector<Point<Dim>>& newPoints, int left_range, int right_range, int curr_dim, KDTreeNode* subroot   ) ;
     void delete_help( KDTreeNode* subroot ) ;
     void help_nn(const Point<Dim> & query, Point<Dim> & best, KDTreeNode* subroot, int curr_dim, bool & initial  ) const ; 
};
