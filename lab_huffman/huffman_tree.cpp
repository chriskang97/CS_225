                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    {
      TreeNode* returned_node = mergeQueue.front() ;
      mergeQueue.pop() ;
    ...
    if ( mergeQueue.empty()   )
    {
      TreeNode* returned_node = singleQueue.front() ;
      singleQueue.pop() ;
      return returned_node ;
    }

    if ( singleQueue.front()->freq < mergeQueue.front()->freq || singleQueue.front()->freq == mergeQueue.front()->freq )
    {
        TreeNode* returned_node = singleQueue.front() ;
        singleQueue.pop() ;
        return returned_node ;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     TreeNode* first_elem ;
     TreeNode* second_elem ;

     // Setup the Single Queue
     for( unsigned i = 0 ; i < frequencies.size() ; i++ )
     {
       TreeNode* curr_elem = new TreeNode(frequencies[i] ) ;
       singleQueue.push(curr_elem) ;
     }

     while( !singleQueue.empty() || mergeQueue.size() != 1 )
     {
       first_elem = removeSmallest(singleQueue, mergeQueue) ;
       second_elem = removeSmallest(singleQueue, mergeQueue) ;

       Frequency* combined_frequency = new Frequency( first_elem->freq.getFrequency() + second_elem->freq.getFrequency()  ) ;
       TreeNode* new_parent = new TreeNode( *combined_frequency ) ;

       if (first_elem->freq < second_elem->freq )
       {
         new_parent->left = first_elem ;
         new_parent->right = second_elem ;
       }

       else
       {
         new_parent->left = second_elem ;
         new_parent->right = first_elem ;
       }

       mergeQueue.push(new_parent) ;
     }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         {
           ss.put(current->freq.getCharacter() ) ;
           current = root_ ;
         }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    {
      Frequency* new_freq = new Frequency(0) ;
      TreeNode* new_internal = new TreeNode(*new_freq) ;
    ...
    {
      Frequency* new_freq = new Frequency(bfile.getNextByte(), 0 ) ;
      TreeNode* new_internal = new TreeNode(*new_freq) ;
      return new_internal ;
    }
