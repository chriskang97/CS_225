...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    unsigned long  left_indx = 0 ;
    unsigned long right_indx = (elements.size() - 1) ;
    int mid_point = (right_indx / 2 ) ;

    // Boundary Conditions
    if ( val < elements.at(left_indx) )
      return 0 ;

    if ( val > elements[right_indx] )
      return right_indx + 1 ;


    while( left_indx != right_indx )
    {
      // Checking if any of the values are equal
      if ( val == elements.at(left_indx) )
        return left_indx ;

      if ( val == elements.at(right_indx) )
        return right_indx ;

      if ( val == elements.at(mid_point) )
        return mid_point ;

      // If val > mid_point adjust left index other wise adjust right_index
      if( val > elements.at(mid_point) )
      {
        left_indx = mid_point ;
        mid_point = (right_indx + left_indx) / 2 ;
      }

      else
      {
        right_indx = mid_point ;
        mid_point = (right_indx + left_indx) / 2 ;
      }

      // Case of 2 values remaining left.
      if ( right_indx - 1 == left_indx )
      {
        if ( val > elements.at(left_indx) )
          return left_indx + 1 ;
